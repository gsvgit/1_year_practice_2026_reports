...
Алгоритм свёртки заключается в следующем: по изображению скользит ядро размером $k\times k$ с шагом 1. Для каждой позиции вычисляется сумма поэлементных произведений пикселей окна и коэффициентов ядра:
\[
        O[i,j] = \sum_{u=0}^{k-1} \sum_{v=0}^{k-1} I_{\text{pad}}[i+u, j+v] \cdot K[u,v],
\]
где:
$O[i,j]$~--- значение для пикселя нового изображения; $I_{\text{pad}}[i+u, j+v]$~--- матрица размером с ядро. Центр матрицы~--- исходный пиксель; $K[u,v]$~--- ядро.
...
На графиках видно, что при росте размера изображения время работы учебной реализации увеличивается пропорционально площади изображения ($O(height\times width)$). Функция cv2.filter2D из OpenCV демонстрирует аналогичную линейную зависимость от площади, но с гораздо меньшей константой (то есть на любом размере изображения OpenCV работает на порядки быстрее).
